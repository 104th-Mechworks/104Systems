...
class MusicPlayerCog(commands.Cog):
    ...
    async def track_back(self, ctx: discord.ApplicationContext):
        ...
        track = (
            ...
            if player.queue.last_track is None
            else player.queue.history.pop(-2)
        )
        ...
        await controller.channel.send(
            embed=discord.Embed(color=discord.Color.blurple()).set_author(
                name=f"{ctx.user.display_name} went back to {track}",
    ...
    async def volume_cmd(
        self,
        ctx: discord.ApplicationContext,
        volume: discord.Option(int, description="Set your desired volume"),  # type: ignore
    ):
        if not ctx.voice_client:
            return await ctx.response.send_message(
    ...
    async def pause(self, ctx: discord.ApplicationContext):
        if not ctx.voice_client:
            return await ctx.response.send_message(
                "You can't pause the queue if I am not playing",
                ephemeral=True,
                delete_after=10,
            )
        player: DragonPlayer = ctx.voice_client
        if (
            ...
            return await ctx.response.send_message(
                f"You may not pause the queue when you are not in the same channel.",
                ephemeral=True,
                delete_after=10,
            )
        ...
        if player.is_paused:
            await ctx.response.send_message(
                f"The queue is already paused", ephemeral=True, delete_after=10
            )
        else:
            await player.set_pause(True)
            await ctx.response.send_message(
                f"Paused the queue", ephemeral=True, delete_after=10
            )
            controller: discord.Message = player.controller
            await controller.channel.send(
                embed=discord.Embed(color=discord.Color.blurple()).set_author(
                    name=f"{ctx.author.display_name} paused the queue",
                    icon_url=ctx.author.display_avatar.url,
                ),
                delete_after=10,
            )
    ...
    async def resume(self, ctx: discord.ApplicationContext):
        if not ctx.voice_client:
            return await ctx.response.send_message(
                "You can't resume the queue if I am not playing",
                ephemeral=True,
                delete_after=10,
            )
        player: DragonPlayer = ctx.voice_client
        if (
            ...
            return await ctx.response.send_message(
                f"You may not resume the queue when you are not in the same channel.",
                ephemeral=True,
                delete_after=10,
            )
        ...
        if not player.is_paused:
            await ctx.response.send_message(
                f"The queue is already playing", ephemeral=True, delete_after=10
            )
        else:
            await player.set_pause(False)
            await ctx.response.send_message(
                f"Resumed the queue", ephemeral=True, delete_after=10
            )
            controller: discord.Message = player.controller
            await controller.channel.send(
                embed=discord.Embed(color=discord.Color.blurple()).set_author(
                    name=f"{ctx.author.display_name} resumed the queue",
                    icon_url=ctx.author.display_avatar.url,
                ),
                delete_after=10,
            )
